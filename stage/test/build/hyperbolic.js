...
export const expi = (t) => ({ re: Math.cos(t), im: Math.sin(t) });
export const MOBIUS_ID = { a: 0, c: { re: 0, im: 0 } };
export function mobiusApply(m, z) {
...
export function mobiusInvert(m) {
 ...
 */
export function mobiusCompose(f, g) {
    const ef = expi(f.a);
    const eg = expi(g.a);
    // f matrix entries
 ...
 */
export function addressToLayout(addr) {
    let lo = 0;
    let width = TAU;
...
export function addressInterval(addr) {
    let lo = 0;
    let width = TAU;
    for (let i = 0; i < addr.length; i++) {
        const k = i === 0 ? 4 : 3;
...
export function diskPoint(theta, rho) {
    const r = Math.tanh(rho / 2);
    return { re: r * Math.cos(theta), im: r * Math.sin(theta) };
}
export function addressToDisk(addr) {
 ...
 */
export function poincareSegmentPoints(z1, z2, n) {
    if (n < 2)
        n = 2;
    ...
    if (d2 < 1e-24 || Math.abs(cross) < 1e-9)
        return straight();
    ...
    if (R2 <= 0)
        return straight(); // numerical guard
...
export function runMathChecks() {
    ...
    ok(thetas.every((t, i) => near(t, Math.PI / 4 + (i * Math.PI) / 2, 1e-12)), 'root children at quarter midpoints');
    ...
    ok(cabs(csub(seg[0], a)) < 1e-9 && cabs(csub(seg[11], b)) < 1e-9, 'geodesic segment endpoints');
