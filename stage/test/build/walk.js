...
export class WalkDriver {
    ...
    onPlayDone = null;
    onCutoff = null; // walk left the renderable range
    ...
    async straighten(fromIndex = 0) {
        const t = this.traj;
        if (!t)
            return { geodesic: [], cut: false };
        const myGen = ++this.gen;
        this.playing = false;
        this.paused = false;
        const seg = t.trail.slice(fromIndex);
        if (seg.length < 2)
            return { geodesic: seg, cut: false }; // nothing pending (sde 0)
        ...
        if (cut || segMaxDepth > MAX_DEPTH) {
            ...
            if (segGeo.length >= 2)
                await this.stage.tracePath(segGeo, 240);
